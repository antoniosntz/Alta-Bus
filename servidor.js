...
async function buscarLocalizacaoTraccar() {
  try {
    ...
    const detalhes = typeof data === 'string' ? data : JSON.stringify(data || {});

    console.error('Erro ao consultar Traccar:', {
      status,
      deviceId: DEVICE_ID,
      url: TRACCAR_URL,
      detalhes,
    ...
    if (status === 400) {
      throw new Error('Traccar retornou 400. Verifique se TRACCAR_DEVICE_ID e TRACCAR_TOKEN pertencem ao mesmo dispositivo/conta.');
    }
...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    // O arquivo motorista.html não existe neste projeto. Por isso, todos os tipos
    // de usuário são enviados para pontos.html, evitando erro e retorno ao login.
    const redirect = 'pontos.html';
    ...
    req.session.save((err) => {
      if (err) {
        ...
        return res.status(500).json({
          success: false,
          message: 'Erro ao salvar sessão de login.',
        });
      }
